package com.houarizegai.spocom.controllers.form;

import com.houarizegai.spocom.controllers.TableAthlete;
import com.houarizegai.spocom.controllers.ModifierCompetitionController;
import com.houarizegai.spocom.controllers.NouveauCompetitionController;
...
public class AthleteFormController implements Initializable {
    ...
    private void addAthlete() { // Add New Student
        if (!nomField.getText().trim().toLowerCase().matches("[a-z0-9_]{4,}")) {            
            toastErrorMsg.enqueue(new JFXSnackbar.SnackbarEvent(new Label("Erreur dans le Nom !")));
            return;
        }
        if (!prenomField.getText().trim().toLowerCase().matches("[a-z0-9_]{4,}")) {
            toastErrorMsg.enqueue(new JFXSnackbar.SnackbarEvent(new Label("Erreur dans le prenom !")));
            return;
        }
        if (dateNaissPicker.getValue() == null) {
            toastErrorMsg.enqueue(new JFXSnackbar.SnackbarEvent(new Label("Please remplir la date de naissance !")));
            return;
        }
        if (!clubField.getText().trim().toLowerCase().matches("[a-z0-9_-]{3,}")) {
            toastErrorMsg.enqueue(new JFXSnackbar.SnackbarEvent(new Label("Erreur dans le club !")));
            return;
        }
        if (comboCodeWilaya.getSelectionModel().getSelectedItem() == null) {
            toastErrorMsg.enqueue(new JFXSnackbar.SnackbarEvent(new Label("Svp, selectionner le code wilaya !")));
            return;
        }
